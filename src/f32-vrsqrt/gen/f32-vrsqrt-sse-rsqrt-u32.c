...
void xnn_f32_vrsqrt_ukernel__sse_rsqrt_u32(
  ...
  for (; batch >= 32 * sizeof(float); batch -= 32 * sizeof(float)) {
    ...
    const __m128 vaSTUV = _mm_loadu_ps(input + 28);
    input += 32;
    ...
    const __m128 vt0_STUV = _mm_rsqrt_ps(vaSTUV);
    ...
    const __m128 vt1_STUV = _mm_mul_ps(vt0_STUV, vt0_STUV);
    ...
    const __m128 vt2_STUV = _mm_mul_ps(vaSTUV, vt1_STUV);
    ...
    const __m128 vt3_STUV = _mm_sub_ps(kThree, vt2_STUV);
    ...
    const __m128 vt4_STUV = _mm_mul_ps(kHalf, vt0_STUV);
    ...
    const __m128 vySTUV = _mm_mul_ps(vt3_STUV, vt4_STUV);
