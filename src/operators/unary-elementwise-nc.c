  ...
  xnn_operator_t* convert_op_out)
{
  ...
  const xnn_float16 fp16_input_scale = xnn_fp16_ieee_from_fp32_value(input_scale);
...
{
  const xnn_float16 alpha_as_half = xnn_fp16_ieee_from_fp32_value(alpha);
  alpha = xnn_fp16_ieee_to_fp32_value(alpha_as_half);
  ...
  if XNN_LIKELY(f16_elu_config != NULL) {
    ...
    f16_elu_config->init.f16_elu(&params,
      /*prescale=*/xnn_fp16_ieee_from_fp32_value(1.0f), alpha_as_half, /*beta=*/xnn_fp16_ieee_from_fp32_value(1.0f));
  }
...
{
  const xnn_float16 negative_slope_as_half = xnn_fp16_ieee_from_fp32_value(negative_slope);
  negative_slope = xnn_fp16_ieee_to_fp32_value(negative_slope_as_half);
