    ...
    xnn_operator_t* average_pooling_op_out)
{
  ...
  avgpool_config->init.f16(&average_pooling_op->params.f16_scaleminmax,
    xnn_fp16_ieee_from_fp32_value(1.0f / (float) (int32_t) pooling_size), fp16_output_min, fp16_output_max);
  ...
  pthreadpool_t threadpool)
{
  ...
  if (is_pixelwise) {
    ...
    average_pooling_op->gavgpool_config->update.f16(
      &average_pooling_op->params.f16_scaleminmax, xnn_fp16_ieee_from_fp32_value(1.0f / (float) (int32_t) input_size));
  }
