  ...
  bool is_pixelwise)
{
  ...
  assert(!is_pixelwise || (avgpool->pavgpool != NULL && indirection_init_pavgpool2d != NULL));
  ...
  if (is_pixelwise) {
    average_pooling_op->context.average_pooling.ukernel = avgpool->pavgpool;
  } else {
    average_pooling_op->context.average_pooling.ukernel = avgpool->avgpool;
  }
