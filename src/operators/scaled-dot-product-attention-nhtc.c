  ...
  xnn_operator_t* attention_op_out)
{
  ...
  if XNN_LIKELY(gemm_config->init.f16 != NULL) {
    gemm_config->init.f16(&minmax_params, xnn_fp16_ieee_from_fp32_value(-INFINITY), xnn_fp16_ieee_from_fp32_value(INFINITY));
  }
...
{
  *output = xnn_fp16_ieee_from_fp32_value(1.0f / xnn_fp16_ieee_to_fp32_value(*input));
}
...
enum xnn_status xnn_reshape_scaled_dot_product_attention_nhtc_f16(
  ...
  pthreadpool_t threadpool)
{
  xnn_float16 cap = xnn_fp16_ieee_from_fp32_value(attention_op->attention.cap_params.cap);
  xnn_float16 cap_reciprocal = xnn_fp16_ieee_from_fp32_value(1.0f / attention_op->attention.cap_params.cap);
