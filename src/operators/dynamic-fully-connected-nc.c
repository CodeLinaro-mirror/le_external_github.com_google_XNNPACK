  ...
  *workspace_size = n_stride * weights_stride;
  ...
  if (packed_lh_config) {
    ...
    if (inline_lhs_packing) {
      ...
      const bool packed_lhs_stays_in_cache = xnn_packed_lhs_stays_in_cache(
          mr, nr,
          /*m_stride=*/input_channels
              << packed_lh_config->log2_input_element_size,
          /*m_packed_stride=*/divide_round_up(per_thread_workspace_size, mr),
          /*n_stride=*/dynamic_fully_connected_op->weights_stride,
          /*cm_stride=*/
          dynamic_fully_connected_op->output_pixel_stride
              << log2_output_element_size,
          /*cn_stride=*/1 << log2_output_element_size,
          /*nc=*/output_channels);
      ...
      } else if (!packed_lhs_stays_in_cache || num_threads * mr > batch_size) {
        xnn_log_debug(
            "Pre-packing LHS of %s with m=%zu, n=%zu, and k=%zu despite "
            "request to inline because %s.",
            xnn_operator_type_to_string(dynamic_fully_connected_op->type),
            batch_size, output_channels, input_channels,
            !packed_lhs_stays_in_cache
  ...
  const size_t nc =
      (packed_lh_config && inline_lhs_packing)
          ...
          : xnn_gemm_best_tile_size(
                ...
                /*m_stride=*/
                gemm_context->gemm.a_stride,
                /*n_stride=*/
                gemm_context->gemm.w_stride,
                /*cm_stride=*/
                gemm_context->gemm.cm_stride,
