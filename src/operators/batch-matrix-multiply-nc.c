...
static enum xnn_status reshape_batch_matrix_multiply_nc(
    ...
    uint32_t log2_lhs_input_element_size, uint32_t log2_rhs_input_element_size,
    uint32_t bias_element_size, uint32_t log2_output_element_size,
    const void* params, size_t params_size, size_t num_threads) {
  ...
  for (int k = 0; k < num_batch_dims; k++) {
    if (batch_dims_c[k] % batch_dims_a[k] != 0 ||
      ...
      xnn_log_error(
          "failed to create %s operator with incompatible %i-th batch "
  ...
  *workspace_size = batch_size_b * rhs_input_batch_stride;
  *workspace_alignment = XNN_ALLOCATION_ALIGNMENT;
  ...
  const size_t w_stride = bias_element_size + (round_up_po2(k, kr * sr)
                                               << log2_lhs_input_element_size);
  const size_t k_scaled = k << log2_lhs_input_element_size;
  ...
  batch_matrix_multiply_op->context.gemm.fused_params = &batch_matrix_multiply_op->context.gemm.params;
    ...
    batch_matrix_multiply_op->compute[1].task_3d_tile_2d = (pthreadpool_task_3d_tile_2d_t) xnn_compute_grouped_gemm;
  #endif
    batch_matrix_multiply_op->compute[1].range[0] = batch_size_c;
