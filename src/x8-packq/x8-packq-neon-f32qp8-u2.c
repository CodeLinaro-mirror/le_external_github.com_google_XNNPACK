...
// This function has been adapted from KleidiAI's
// `kai_run_lhs_quant_pack_qai8dxp_f32` as a reference scalar implementation.

void xnn_x8_packq_f32qp8_ukernel__neon_u2(size_t m, size_t k, size_t mr,
                                          size_t kr, size_t sr,
                                          size_t m_idx_start,
...
#else
      float32x2_t vmin = vpmin_f32(vget_low_f32(vmin0), vget_high_f32(vmin0));
      min0 = vget_lane_f32(vpmin_f32(vmin, vmin), 0);
      float32x2_t vmax = vpmax_f32(vget_low_f32(vmax0), vget_high_f32(vmax0));
      max0 = vget_lane_f32(vpmax_f32(vmax, vmax), 0);
