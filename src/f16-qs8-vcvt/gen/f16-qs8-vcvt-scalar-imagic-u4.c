    ...
    const struct xnn_f16_qs8_cvt_params params[restrict XNN_MIN_ELEMENTS(1)])
{
  ...
  const float vscale = xnn_fp16_ieee_to_fp32_value(params->scalar.scale);
  ...
  for (; batch >= 4 * sizeof(xnn_float16); batch -= 4 * sizeof(xnn_float16)) {
    float vx0 = xnn_fp16_ieee_to_fp32_value(i[0]);
    float vx1 = xnn_fp16_ieee_to_fp32_value(i[1]);
    float vx2 = xnn_fp16_ieee_to_fp32_value(i[2]);
    float vx3 = xnn_fp16_ieee_to_fp32_value(i[3]);
    i += 4;
  ...
  if XNN_UNLIKELY(batch != 0) {
    do {
      float vx = xnn_fp16_ieee_to_fp32_value(*i++);
      vx *= vscale;
      vx += vmagic_bias;
