...
// type in C++.
struct xnn_float16_t {
  uint16_t value;

#ifdef __cplusplus
  xnn_float16_t() = default;
  xnn_float16_t(float x) : value(fp16_ieee_from_fp32_value(x)) {}

  operator float() const { return fp16_ieee_to_fp32_value(value); }
#endif
};
typedef struct xnn_float16_t xnn_float16;

struct xnn_bfloat16_t {
  uint16_t value;

#ifdef __cplusplus
  xnn_bfloat16_t() = default;
  xnn_bfloat16_t(float x) : value(math_cvt_bf16_fp32(x)) {}

  operator float() const { return math_cvt_fp32_bf16(value); }
#endif
};
typedef struct xnn_bfloat16_t xnn_bfloat16;
...
XNN_INLINE static xnn_float16 xnn_fp16_ieee_from_fp32_value(float f) {
  struct xnn_float16_t result;
  result.value = fp16_ieee_from_fp32_value(f);
  return result;
}

XNN_INLINE static float xnn_fp16_ieee_to_fp32_value(xnn_float16 fp16) {
  return fp16_ieee_to_fp32_value(fp16.value);
}

XNN_INLINE static xnn_bfloat16 xnn_bf16_from_fp32_value(float f) {
  xnn_bfloat16 result;
  result.value = math_cvt_bf16_fp32(f);
  return result;
}

XNN_INLINE static float xnn_bf16_to_fp32_value(xnn_bfloat16 bf16) {
