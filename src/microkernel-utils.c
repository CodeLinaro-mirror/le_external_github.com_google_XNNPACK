...
bool xnn_packed_lhs_stays_in_cache(const struct xnn_gemm_config *gemm_config,
                                   size_t m_stride, size_t m_packed_stride,
                                   size_t n_stride, size_t cn_stride, size_t mc,
                                   size_t nc) {
  ...
  // Compute the amount of cache needed for the packed lhs.
  const size_t packed_lhs_cache_bytes =
      round_up_po2(mc * m_packed_stride, cache_line_size);

  // Compute the amount of cache needed for both a single GEMM tile, and for a
  // row of GEMM tiles.
  const size_t gemm_tile_cache_bytes =
      round_up_po2(mr * m_stride, cache_line_size) +
      round_up_po2(nr * n_stride, cache_line_size) +
      mr * round_up_po2(nr * cn_stride, cache_line_size);
  const size_t gemm_row_cache_bytes =
      round_up_po2(mr * m_stride, cache_line_size) +
      round_up_po2(nc * n_stride, cache_line_size) +
      mr * round_up_po2(nc * cn_stride, cache_line_size);

  xnn_log_info(
      "mr=%zu, nr=%zu, m_stride=%zu, m_packed_stride=%zu, n_stride=%zu, "
      "mc=%zu, nc=%zu, packed_lhs_cache_bytes=%zu, gemm_tile_cache_bytes=%zu, "
      "gemm_row_cache_bytes=%zu, inline_lhs_packing=%s.",
      mr, nr, m_stride, m_packed_stride, n_stride, mc, nc,
      packed_lhs_cache_bytes, gemm_tile_cache_bytes, gemm_row_cache_bytes,
      (cache_bytes < packed_lhs_cache_bytes + gemm_tile_cache_bytes ||
       gemm_row_cache_bytes < cache_bytes)
          ? "true"
          : "false");

  // If the packed LHS data and a single tile are enough to flood the cache,
  // then pre-packing the LHS is never a good idea, so definitely inline the
  // packing (but not on `sme2`).
#if XNN_ARCH_ARM64
  if (!(gemm_config->arch & xnn_arch_arm_sme2) &&
      cache_bytes < packed_lhs_cache_bytes + gemm_tile_cache_bytes) {
#else
  if (cache_bytes < packed_lhs_cache_bytes + gemm_tile_cache_bytes) {
#endif  // XNN_ARCH_ARM64
    return true;
  }

  // Otherwise, we only want to inline the LHS packing if it's possible to
  // compute an entire row of the GEMM without flooding the cache.
  return gemm_row_cache_bytes < cache_bytes;
}
