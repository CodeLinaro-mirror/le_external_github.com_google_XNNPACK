  ...
  if (tensorflow_legacy || align_corners) {
    for (size_t output_y = output_y_start; output_y < output_y_end; output_y++) {
      ...
      for (size_t output_x = 0; output_x < output_width; output_x++) {
        ...
        w[0] = xnn_fp16_ieee_from_fp32_value(alpha_x);
        w[1] = xnn_fp16_ieee_from_fp32_value(alpha_y);
  ...
  xnn_float16* pixelwise_buffer)
{
  for (size_t output_y = 0; output_y < output_height; output_y++) {
    ...
    for (size_t output_x = 0; output_x < output_width; output_x++) {
      ...
      *pixelwise_buffer++ = xnn_fp16_ieee_from_fp32_value(1.0f / ((float) (int32_t) (input_y_range * input_x_range)));
    }
  }
}
