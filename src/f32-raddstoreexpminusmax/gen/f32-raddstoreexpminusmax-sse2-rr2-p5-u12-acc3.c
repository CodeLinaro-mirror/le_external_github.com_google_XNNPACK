...
void xnn_f32_raddstoreexpminusmax_ukernel__sse2_rr2_p5_u12_acc3(
    size_t batch,
    const float* input,
    const float* max,
    float* output,
    ...
    const union xnn_f32_expminus_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  XNN_FORCE_STACK const __m128 vlog2e = _mm_set1_ps(0x1.715476p+0f);
  ...
  XNN_FORCE_STACK const __m128 vdenorm_cutoff = _mm_set1_ps(-0x1.5D589Ep6f);
