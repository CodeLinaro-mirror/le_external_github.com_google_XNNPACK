...
void xnn_f32_raddstoreexpminusmax_ukernel__wasmrelaxedsimd_rr2_p5_u20_acc2(
    size_t batch,
    const float* input,
    const float* max,
    float* output,
    ...
    const union xnn_f32_expminus_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  XNN_FORCE_STACK const v128_t vlog2e = wasm_f32x4_const_splat(0x1.715476p+0f);
  XNN_FORCE_STACK const v128_t vmagic_bias = wasm_f32x4_const_splat(0x1.8000FEp23f);
  XNN_FORCE_STACK const v128_t vminus_ln2_hi = wasm_f32x4_const_splat(-0x1.62E400p-1f);
  XNN_FORCE_STACK const v128_t vminus_ln2_lo = wasm_f32x4_const_splat(-0x1.7F7D1Cp-20f);
  ...
  XNN_FORCE_STACK const v128_t vdenorm_cutoff = wasm_f32x4_const_splat(-0x1.5D589Ep6f);
