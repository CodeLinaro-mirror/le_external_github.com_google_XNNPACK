...
TEST(GetExecutionPlan, MultipleOutputs) {
  auto op_a = std::make_shared<NoOp>();
  TensorHandle a_handle = AddOutput(op_a, std::source_location::current());
...
TEST(GetExecutionPlan, CycleDetection) {
  ...
  auto tensor_a = graph::NewTensor(std::source_location::current());
  auto tensor_b = graph::NewTensor(std::source_location::current());
  auto op_a = std::make_shared<NoOp>();
  auto op_b = std::make_shared<NoOp>();
