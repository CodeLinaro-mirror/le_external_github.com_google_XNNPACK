...
TEST(DotSchedulingTest, PackBFusedWithDot) {
  ...
  auto last_pack_b_it = trace_events.end();
  for (auto it = trace_events.rbegin(); it != trace_events.rend(); ++it) {
    if (contains(*it, "pack_b")) {
      last_pack_b_it = it.base() - 1;
      break;
    }
  }
...
TEST(DotSchedulingTest, PackBHoistedForMultiK) {
  ...
  std::vector<std::string> trace_events;
  std::set<std::thread::id> pack_b_threads;
  std::mutex trace_mutex;  // NOLINT(build/c++11)
  runtime.get()->eval_config.trace_begin =
      [&](const char* name) -> slinky::index_t {
    std::lock_guard<std::mutex> lock(trace_mutex);  // NOLINT(build/c++11)
    trace_events.push_back(name);
    if (contains(name, "pack_b")) {
      pack_b_threads.insert(std::this_thread::get_id());
    }
    return 0;
  };
  ...
  EXPECT_GT(scheduler.task_count(), 0);
  EXPECT_GT(pack_b_threads.size(), 1);
}
