...
def print_polynomial(name, coeffs, output_multiplier=False):
  if coeffs.dtype == np.float32:
    precision, wrapper_start, wrapper_end = 10, "", ""
  else:
    precision, wrapper_start, wrapper_end = 18, "f64(", ")"
  rev_coeffs = coeffs[::-1]
  ...
  for i, val in enumerate(rev_coeffs):
    ...
    print(f"      {wrapper_start}{val:.{precision}e}{wrapper_end}{multiplier_str}{comma}")
  print("  ]")
...
# fp32 exp2m1
f = lambda x: np.expm1(x*math.log(2))
p_degree, q_degree = 3, 3
x_min, x_max = -0.5, 0.5
# Use force_zero=True to ensure exp2m1(0) = 0
p, q = rational_approximation(
...
# Target: erf(sqrt(x)) / sqrt(x)
# This transformation forces erf(t) to have an odd numerator and even denominator when t^2 = x.
def f_target_erf(x):
    sqrt_x = np.sqrt(np.maximum(x, 1e-15))
    return erf(sqrt_x) / sqrt_x

p_degree, q_degree = 5, 5
x_min, x_max = 0, 3.8**2  # erf(4) is very close to 1

p, q = rational_approximation(
    f_target_erf, x_min, x_max, p_degree, q_degree, dtype=np.float32
)

# Coefficients for erf(t) = t * (p0 + p1*t^2 + ...) / (q0 + q1*t^2 + ...)
print_polynomial("p", p, output_multiplier=True)
print_polynomial("q", q)

# Evaluate final error for erf(t)
t_test = np.linspace(-4, 4, 5000).astype(np.float32)
x_test = t_test**2
approx = t_test * (poly_eval(p, x_test) / poly_eval(q, x_test))
...
plot_error(
    ...
    t_test,
    approx,
    title="Relative Error for symmetric erf(t) via erf(sqrt(x))/sqrt(x)"
)
# %%
# fp32 expm1
