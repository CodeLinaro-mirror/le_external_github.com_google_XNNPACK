...
void quantize_fp32_to_int8(size_t m, size_t n, size_t stride_a_m,
  ...
  quantize<int8_t>(m, n, stride_a_m, stride_a_n,
                   reinterpret_cast<const float*>(a), stride_b_m, stride_b_n,
                   reinterpret_cast<const float*>(b), stride_c_m, stride_c_n,
                   reinterpret_cast<const int32_t*>(c), stride_x_m,
                   reinterpret_cast<int8_t*>(x), params);
}
...
void quantize_fp32_to_uint8(size_t m, size_t n, size_t stride_a_m,
                           ...
                           size_t stride_c_n, const void* c, size_t stride_x_m,
                           void* x, const ternary_params* params) {
  quantize<uint8_t>(m, n, stride_a_m, stride_a_n,
                    reinterpret_cast<const float*>(a), stride_b_m, stride_b_n,
                    reinterpret_cast<const float*>(b), stride_c_m, stride_c_n,
                    reinterpret_cast<const int32_t*>(c), stride_x_m,
                    reinterpret_cast<uint8_t*>(x), params);
}
