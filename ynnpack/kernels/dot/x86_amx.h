...
inline uint64_t make_amx_config_key(uint8_t kernel_id, uint8_t sizeof_tab,
                                    uint8_t sizeof_tc, uint8_t align_k,
                                    size_t m, size_t n, size_t k_len) {
  return (static_cast<uint64_t>(m & 0xFFFF) << 48) |
         (static_cast<uint64_t>(n & 0xFFFF) << 32) |
         (static_cast<uint64_t>(k_len & 0xFF) << 24) |
         (static_cast<uint64_t>(align_k & 0xFF) << 16) |
         (static_cast<uint64_t>(sizeof_tc & 0xF) << 12) |
         (static_cast<uint64_t>(sizeof_tab & 0xF) << 8) |
         static_cast<uint64_t>(kernel_id);
}
...
struct AmxConfigState {
  ...
  AmxConfigState() = default;

  AmxConfigState(const AmxConfigState&) = delete;
  AmxConfigState& operator=(const AmxConfigState&) = delete;
  AmxConfigState(AmxConfigState&&) = delete;
  AmxConfigState& operator=(AmxConfigState&&) = delete;

  // An all-zero config / current_key == 0 signals unconfigured.
  bool is_configured() const { return current_key != 0; }

  void load(const tile_config& config, uint64_t key) {
    _tile_loadconfig(&config);
    current_config = config;
    current_key = key;
  }

  void load_if_needed(const tile_config& desired, uint64_t key) {
    if (current_key != key) {
      load(desired, key);
    }
  }
...
inline void init_amx_state(dot_kernel_state* state) {
  static_assert(sizeof(AmxConfigState) <= sizeof(state->storage));
  static_assert(alignof(AmxConfigState) <= alignof(dot_kernel_state));
  static_assert(offsetof(dot_kernel_state, storage) % alignof(AmxConfigState) ==
                0);
  // Stack allocation. Constructs `AmxConfigState` in `state->storage`.
  new (state->storage) AmxConfigState();
  state->destroy_fn = [](dot_kernel_state* s) {
    s->as<AmxConfigState>()->~AmxConfigState();
  };
}
...
static void load_tile_config_1x4(size_t m, size_t n, size_t ktail,
  ...
  constexpr size_t align_k = 4 / sizeof(TA);
  // We use `14` as the kernel ID for 1x4 tiles.
  const uint64_t key =
      make_amx_config_key(14, sizeof(TA), sizeof(TC), align_k, m, n, ktail);
  if (amx_state.current_key == key) {
    return;
  }
  ...
  amx_state.load_if_needed(config, key);
}
...
static void load_config_2x2(size_t m, size_t n, size_t k_len, size_t align_k,
                            AmxConfigState& amx_state) {
  const uint64_t key =
      make_amx_config_key(22, sizeof(TAB), sizeof(TC), align_k, m, n, k_len);
  if (amx_state.current_key == key) {
    return;
  }
...
static void load_config_2x3(size_t m, size_t k_len, size_t align_k,
                            AmxConfigState& amx_state) {
  const uint64_t key =
      make_amx_config_key(23, sizeof(TAB), sizeof(TC), align_k, m, 48, k_len);
  if (amx_state.current_key == key) {
    return;
  }
