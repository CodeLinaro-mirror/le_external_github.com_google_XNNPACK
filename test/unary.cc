    ...
    for (const auto& i : EnumerateIndices(output.extents())) {
      if (std::is_integral<Out>::value) {
        if (std::is_integral<In>::value) {
          const int32_t expected = op_info->ReferenceImpl(input(i), params);
          ASSERT_EQ(expected, output(i))
              << "i = " << index_to_string(i) << ", input(i) = " << input(i);
        } else {
          // Integral output, non-integral input. We need to potentially
          // dequantize the input, and avoid UB when converting to int.
          const int32_t expected =
              round_float_to_int<Out>(op_info->ReferenceImpl(
                  dequantize(input(i), input_quantization), params));
          ASSERT_EQ(expected, output(i))
              << "i = " << index_to_string(i) << ", input(i) = " << input(i);
        }
...
const xnn_datatype all_datatypes[] = {
    xnn_datatype_int32, xnn_datatype_quint8, xnn_datatype_qint8,
#ifndef XNN_EXCLUDE_F16_TESTS
    xnn_datatype_fp16,
#endif
    xnn_datatype_bf16,  xnn_datatype_fp32,
