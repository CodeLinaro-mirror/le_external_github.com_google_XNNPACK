  ...
  xnn_pack_qs8_igemm_fn pack) const
{
  ...
  std::vector<xnn_float16_t> c((mr() - 1) * cm_stride() + ((n() - 1) / nr()) * cn_stride() + (n() - 1) % nr() + 1);
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    igemm(m(), n(), k(), ks() * mr() * sizeof(void*),
        ...
        c.data(), cm_stride() * sizeof(xnn_float16_t), cn_stride() * sizeof(xnn_float16_t),
  ...
  xnn_pack_qs8_gemm_fn pack) const
{
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    gemm(m(), n(), k(),
        ...
        c.data(), cm_stride() * sizeof(xnn_float16_t), cn_stride() * sizeof(xnn_float16_t), &params, quantization_params.data());
  ...
  xnn_pack_qs8_qb4w_gemm_fn pack) const
{
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    pack(/*g=*/1, n(), k2, nr(), kr(), sr(), bl(),
      ...
      packed_w.data(), sizeof(xnn_float16_t) * nr(), sizeof(float) * nr(), &packing_params);

    // Fill in packed kernel scale
    size_t stride =  nr() * (packed_k_bytes + /* scales= */ num_blocks * sizeof(xnn_float16_t) + /* ksum= */ sizeof(float) + /* bias= */ sizeof(float));
    size_t block_stride = (bl() / 2 + sizeof(xnn_float16_t)) * nr();
  ...
  xnn_pack_x16_gemm_fn pack) const
{
  ...
  std::vector<xnn_bfloat16_t> a((m() - 1) * a_stride() + k() + XNN_EXTRA_BYTES / sizeof(xnn_bfloat16_t));
  std::vector<xnn_bfloat16_t> b(n() * k());
  std::vector<xnn_bfloat16_t, AlignedAllocator<xnn_bfloat16_t, XNN_ALLOCATION_ALIGNMENT>> packed_w(packed_n() * packed_k() + packed_n());
  std::vector<xnn_bfloat16_t> bias(n());
  std::vector<xnn_bfloat16_t> c((mr() - 1) * cm_stride() + ((n() - 1) / nr()) * cn_stride() + (n() - 1) % nr() + 1);
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    const float accumulated_min = *std::min_element(c_ref.cbegin(), c_ref.cend());
    const float accumulated_max = *std::max_element(c_ref.cbegin(), c_ref.cend());
    const float c_min = xnn_bfloat16_t(accumulated_min + (accumulated_max - accumulated_min) / 255.0f * float(qmin()));
    const float c_max = xnn_bfloat16_t(accumulated_max - (accumulated_max - accumulated_min) / 255.0f * float(255 - qmax()));
    ...
    gemm_minmax(m(), n(), k() * sizeof(xnn_bfloat16_t),
      a.data(), a_stride() * sizeof(xnn_bfloat16_t),
      packed_w.data(),
      c.data(), cm_stride() * sizeof(xnn_bfloat16_t), cn_stride() * sizeof(xnn_bfloat16_t),
      &params);
  ...
  xnn_pack_x16_gemm_fn pack) const
{
  ...
  std::vector<xnn_float16_t> a((m() - 1) * a_stride() + k() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<xnn_float16_t> b(n() * k());
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, XNN_ALLOCATION_ALIGNMENT>> packed_w(packed_n() * packed_k() + packed_n());
  std::vector<xnn_float16_t> bias(n());
  std::vector<xnn_float16_t> c((mr() - 1) * cm_stride() + ((n() - 1) / nr()) * cn_stride() + (n() - 1) % nr() + 1);
  std::vector<float> c_ref(m() * n());
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    const float accumulated_min = *std::min_element(c_ref.cbegin(), c_ref.cend());
    const float accumulated_max = *std::max_element(c_ref.cbegin(), c_ref.cend());
    const float c_min = xnn_float16_t(accumulated_min + (accumulated_max - accumulated_min) / 255.0f * float(qmin()));
    const float c_max = xnn_float16_t(accumulated_max - (accumulated_max - accumulated_min) / 255.0f * float(255 - qmax()));
    ...
    gemm_minmax(m(), n(), k() * sizeof(xnn_float16_t),
      a.data(), a_stride() * sizeof(xnn_float16_t),
      packed_w.data(),
      c.data(), cm_stride() * sizeof(xnn_float16_t), cn_stride() * sizeof(xnn_float16_t),
      &params);
  ...
  xnn_pack_x16_igemm_fn pack) const
{
  ...
  std::vector<xnn_float16_t> a((mr() - 1) * a_stride() + k() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<xnn_float16_t> b(n() * ks() * k());
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, XNN_ALLOCATION_ALIGNMENT>> packed_w(ks() * packed_k() * packed_n() + packed_n());
  std::vector<xnn_float16_t> bias(n());
  std::vector<xnn_float16_t> c((mr() - 1) * cm_stride() + ((n() - 1) / nr()) * cn_stride() + (n() - 1) % nr() + 1);
  std::vector<float> c_ref(m() * n());
  std::vector<xnn_float16_t> junk(k() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<const xnn_float16_t*> im2col(mr() * ks());
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    const float accumulated_min = *std::min_element(c_ref.cbegin(), c_ref.cend());
    const float accumulated_max = *std::max_element(c_ref.cbegin(), c_ref.cend());
    const float c_min = xnn_float16_t(accumulated_min + (accumulated_max - accumulated_min) / 255.0f * xnn_float16_t(qmin()));
    const float c_max = xnn_float16_t(accumulated_max - (accumulated_max - accumulated_min) / 255.0f * xnn_float16_t(255 - qmax()));
    for (size_t m_index = 0; m_index < m(); m_index++) {
      for (size_t n_index = 0; n_index < n(); n_index++) {
    ...
    const xnn_float16_t* zero_pointer = (zero_index() != SIZE_MAX) ? a.data() : nullptr;

    igemm_minmax(
      m(), n(), k() * sizeof(xnn_float16_t), ks() * mr() * sizeof(void*),
      reinterpret_cast<const xnn_float16_t**>(im2col.data()), packed_w.data(),
      c.data(), cm_stride() * sizeof(xnn_float16_t), cn_stride() * sizeof(xnn_float16_t),
      a_offset() * sizeof(xnn_float16_t), zero_pointer,
      &params);
