...
class GlobalAveragePoolingOperatorTester {
 public:
  ...
  void TestNWCxF16() const {
    ...
    std::vector<xnn_float16_t> input((batch_size() * width() - 1) * input_stride() + channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> output(batch_size() * output_stride());
    std::vector<float> output_ref(batch_size() * channels());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      std::unique_ptr<pthreadpool, decltype(&pthreadpool_destroy)> auto_threadpool{nullptr, pthreadpool_destroy};
      if (multithreaded()) {
  ...
  void TestNCWxF16() const {
    ...
    std::vector<xnn_float16_t> input(batch_size() * channels() * width() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> output(batch_size() * channels());
    std::vector<float> output_ref(batch_size() * channels());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      std::unique_ptr<pthreadpool, decltype(&pthreadpool_destroy)> auto_threadpool{nullptr, pthreadpool_destroy};
      if (multithreaded()) {
