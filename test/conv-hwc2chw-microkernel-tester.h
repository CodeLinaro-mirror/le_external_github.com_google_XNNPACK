 ...
 public:
  ...
  void Test(xnn_f16_conv_hwc2chw_ukernel_fn conv, xnn_init_f16_minmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> input(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
      batch_size() * ((input_height() * input_width() - 1) * input_pixel_stride() + input_channels()));
    std::vector<xnn_float16_t> zero(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + input_width() * input_channels());
    std::vector<xnn_float16_t> kernel(output_channels() * kernel_height() * kernel_width() * input_channels());
    std::vector<xnn_float16_t> bias(output_channels());
    std::vector<xnn_float16_t> output(batch_size() * output_channels() * output_height() * output_width());
    std::vector<float> output_ref(batch_size() * output_channels() * output_height() * output_width());
    std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> packed_weights((input_channels() * kernel_height() * kernel_width() + 1) * packed_output_channels());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const float output_min = xnn_float16_t(accumulated_min + accumulated_range / 255.0f * float(qmin()));
      const float output_max = xnn_float16_t(accumulated_max - accumulated_range / 255.0f * float(255 - qmax()));
      ...
      conv(
        ...
        input.data(), zero.data(), packed_weights.data(), output.data(),
        padding_top(), output_channels(),
        output_width() * sizeof(xnn_float16_t),
        output_height() * output_width() * sizeof(xnn_float16_t),
        &params);
