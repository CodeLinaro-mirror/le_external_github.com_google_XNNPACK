...
class VCMulMicrokernelTester {
 public:
  ...
  void Test(xnn_f16_vbinary_ukernel_fn vcmul, xnn_init_f16_default_params_fn init_params = nullptr) const {
    ...
    std::vector<xnn_float16_t> a(2 * batch_size() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> b(2 * batch_size() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> y(2 * batch_size() + (inplace_a() || inplace_b() ? XNN_EXTRA_BYTES / sizeof(xnn_float16_t) : 0));
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      vcmul(batch_size() * sizeof(xnn_float16_t), a_data, b_data, y.data(), init_params != nullptr ? &params : nullptr);
