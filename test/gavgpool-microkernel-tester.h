...
class GAvgPoolMicrokernelTester {
 public:
  ...
  void Test(xnn_f16_gavgpool_minmax_unipass_ukernel_fn gavgpool_minmax, xnn_init_f16_scaleminmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> input((rows() - 1) * input_stride() + channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> zero(channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> output(channels());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const float output_min = xnn_float16_t(accumulated_min + float(qmin()) / 255.0f * accumulated_range);
      const float output_max = xnn_float16_t(accumulated_max - float(255 - qmax()) / 255.0f * accumulated_range);
      ...
      // Call optimized micro-kernel.
      gavgpool_minmax(rows(), channels(),
        input.data(), input_stride() * sizeof(xnn_float16_t),
        zero.data(),
        output.data(),
        &params);
  ...
  void Test(xnn_f16_gavgpool_minmax_multipass_ukernel_fn gavgpool_minmax, xnn_init_f16_scaleminmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> input((rows() - 1) * input_stride() + channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> buffer(channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> zero(channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> output(channels());
    std::vector<float> output_ref(channels());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
