...
class SumOperatorTester {
 public:
  SumOperatorTester& input_shape(std::initializer_list<size_t> input_shape) {
    assert(input_shape.size() <= XNN_MAX_TENSOR_DIMS);
    this->input_shape_ = std::vector<size_t>(input_shape);
    return *this;
  }

  SumOperatorTester& input_shape(const std::vector<size_t>& input_shape) {
    assert(input_shape.size() <= XNN_MAX_TENSOR_DIMS);
    this->input_shape_ = std::vector<size_t>(input_shape);
    return *this;
  }
  ...
  SumOperatorTester& reduction_axes(std::initializer_list<size_t> reduction_axes) {
    assert(reduction_axes.size() <= XNN_MAX_TENSOR_DIMS);
    this->reduction_axes_ = std::vector<size_t>(reduction_axes);
    return *this;
  }

  SumOperatorTester& reduction_axes(const std::vector<size_t> reduction_axes) {
    assert(reduction_axes.size() <= XNN_MAX_TENSOR_DIMS);
  ...
  SumOperatorTester& multithreaded(size_t multithreaded) {
  ...
  SumOperatorTester& iterations(size_t iterations) {
  ...
  void TestF16() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      // Create, setup, run, and destroy a sum operator.
      ASSERT_EQ(xnn_status_success, xnn_initialize(nullptr /* allocator */));
      xnn_operator_t sum_op = nullptr;

      const xnn_status status = xnn_create_sum_nd_f16(
          /*flags=*/0, &sum_op);
      if (status == xnn_status_unsupported_hardware) {
        GTEST_SKIP();
      }
      ASSERT_EQ(xnn_status_success, status);
      ASSERT_NE(nullptr, sum_op);

      // Smart pointer to automatically delete sum_op.
      std::unique_ptr<xnn_operator, decltype(&xnn_delete_operator)> auto_sum_op(sum_op, xnn_delete_operator);

      size_t workspace_size = SIZE_MAX;
      size_t workspace_alignment = SIZE_MAX;
      ASSERT_EQ(xnn_status_success,
        xnn_reshape_sum_nd_f16(
          sum_op,
          num_reduction_axes(),
          reduction_axes().data(),
          num_input_dims(),
      ...
      ASSERT_EQ(xnn_status_success,
        xnn_setup_sum_nd_f16(
          sum_op,
          workspace.data(),
          input.data(), output.data()));

      ASSERT_EQ(xnn_status_success,
        xnn_run_operator(sum_op, auto_threadpool.get()));
  ...
  void TestF32() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const xnn_status status = xnn_create_sum_nd_f32(
          /*flags=*/0, &sum_op);
      if (status == xnn_status_unsupported_hardware) {
        GTEST_SKIP();
      }
  ...
  void TestQS8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t idx = 0; idx < output_ref.size(); ++idx) {
        ...
        output_ref[idx] *= input_scale * output_scale;
