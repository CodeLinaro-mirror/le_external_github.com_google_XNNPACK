...
template <typename Bias, typename Scale>
double GetBiasValue(Tensor<Bias> bias, size_t n,
  ...
  double bias_value = 0.0f;
  if (!bias.empty()) {
    if (xnn_datatype_is_channelwise_quantized(datatype_of<Bias>())) {
      // If the bias is channelwise quantized, take the scaling from this
      // channel in the filter.
      xnn_quantization_params filter_quantization = {filter_zero_point,
                                                     filter_scale(n, 0)};
      bias_value = dequantize(
          bias(n),
          {.zero_point = bias_quantization.zero_point,
           .scale = input_quantization.scale * filter_quantization.scale});
    } else {
      bias_value = dequantize(bias(n), bias_quantization);
    }
  }
  return bias_value;
}
...
void MatrixVectorMultiply(const Input* input, const Tensor<Filter>& filter,
  ...
  for (size_t oc = 0; oc < filter.extent(0); ++oc) {
    double output_ic =
        GetBiasValue(bias, oc, input_quantization, filter_zero_point,
                     filter_scale, bias_quantization);
...
void TestStaticB(xnn_datatype convert_to = xnn_datatype_invalid,
  ...
  for (auto _ : FuzzTest(std::chrono::milliseconds(500))) {
    ...
    Tensor<Bias> bias;
    if (!std::is_same<Bias, invalid_type>::value && rng() & 1) {
