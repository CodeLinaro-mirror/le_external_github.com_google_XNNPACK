...
class AvgPoolMicrokernelTester {
 public:
  ...
  void Test(xnn_f16_avgpool_minmax_unipass_ukernel_fn avgpool_minmax, xnn_init_f16_scaleminmax_params_fn init_params) const {
    ...
    std::vector<const xnn_float16_t*> indirect_input((output_pixels() - 1) * step() + packed_pooling_elements());
    ...
    std::vector<xnn_float16_t> output((output_pixels() - 1) * output_stride() + channels());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      std::fill(input.end() - XNN_EXTRA_BYTES / sizeof(xnn_float16_t), input.end(), std::nanf(""));
      ...
      for (size_t x = 0; x < output_pixels(); x++) {
        for (size_t c = 0; c < channels(); c++) {
          ...
          for (size_t p = 0; p < pooling_elements(); p++) {
            const xnn_float16_t* row = indirect_input[x * step() + p];
            if (row != zero.data()) {
      ...
      const xnn_float16_t output_min_as_half = output_min_as_float;
      const xnn_float16_t output_max_as_half = output_max_as_float;
      ...
      avgpool_minmax(output_pixels(), pooling_elements(), channels(),
        reinterpret_cast<const xnn_float16_t**>(indirect_input.data()), input_offset() * sizeof(xnn_float16_t), zero.data(),
        output.data(),
        step() * sizeof(void*),
  ...
  void Test(xnn_f16_avgpool_minmax_multipass_ukernel_fn avgpool_minmax, xnn_init_f16_scaleminmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> output((output_pixels() - 1) * output_stride() + channels());
    std::vector<float> output_ref(output_pixels() * channels());
    std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> buffer(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + channels());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      std::generate(input.begin(), input.end(), [&]() { return f32dist(rng); });
      std::fill(input.begin(), input.begin() + input_offset(), std::nanf(""));
      std::fill(input.end() - XNN_EXTRA_BYTES / sizeof(xnn_float16_t), input.end(), std::nanf(""));
  ...
  void Test(xnn_f16_pavgpool_minmax_unipass_ukernel_fn pavgpool_minmax, xnn_init_f16_scaleminmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> input(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
      input_offset() + indirect_input.size() * channels());
    std::vector<xnn_float16_t> zero(channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> multiplier(output_pixels());
    std::vector<xnn_float16_t> output((output_pixels() - 1) * output_stride() + channels());
    std::vector<float> output_ref(output_pixels() * channels());
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      std::generate(input.begin(), input.end(), [&]() { return f32dist(rng); });
