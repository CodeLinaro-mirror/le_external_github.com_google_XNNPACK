...
class BatchMatMulOperatorTester {
 public:
  ...
  void TestF16() const {
    ...
    size_t batch_dims_output[XNN_MAX_TENSOR_DIMS];
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      if (num_batch_dims == 1) {
        if (transpose_b()) {
          ...
          for (size_t b = 0; b < batch_size_output; b++) {
                ...
                for (size_t ki = 0; ki < k(); ki++) {
                  output_ref[b * m() * n() + mi * n() + ni] +=
                      fp16_ieee_to_fp32_value(
                          input_a[ba * m() * k() + mi * k() + ki]) *
                      fp16_ieee_to_fp32_value(
                          input_b[bb * n() * k() + ni * k() + ki]);
                }
              }
            }
          }
        ...
        if (transpose_b()) {
          ...
          for (size_t b0 = 0; b0 < batch_dims_output[0]; b0++) {
            ...
            for (size_t b1 = 0; b1 < batch_dims_output[1]; b1++) {
              const size_t ba =
                  b0a * batch_dims_a()[1] + (b1 % batch_dims_a()[1]);
              const size_t bb =
                  b0b * batch_dims_b()[1] + (b1 % batch_dims_b()[1]);
              const size_t bc = b0 * batch_dims_output[1] + b1;
              for (size_t mi = 0; mi < m(); mi++) {
                for (size_t ni = 0; ni < n(); ni++) {
                  for (size_t ki = 0; ki < k(); ki++) {
                    output_ref[bc * m() * n() + mi * n() + ni] +=
                        fp16_ieee_to_fp32_value(
                            input_a[ba * m() * k() + mi * k() + ki]) *
                        fp16_ieee_to_fp32_value(
                            input_b[bb * n() * k() + ni * k() + ki]);
