...
class BatchMatMulOperatorTester {
 public:
  ...
  static void ComputeRefF16(size_t m, size_t k, size_t n, bool transpose_b,
                            const xnn_float16_t* input_a, const xnn_float16_t* input_b,
                            float* output_ref) {
    if (transpose_b) {
      // lhs is B*M*K, rhs is B*N*K.
      for (size_t mi = 0; mi < m; mi++) {
  ...
  void TestF16() const {
    ...
    std::vector<xnn_float16_t> input_a(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
                                  batch_size_a * m() * k());
    std::vector<xnn_float16_t> input_b(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
                                  batch_size_b * k() * n());
    std::vector<xnn_float16_t> output(batch_size_output * m() * n());
