...
namespace xnnpack {
template <class T>
class SumTestBase : public ::testing::TestWithParam<bool> {
 protected:
  SumTestBase() {
  ...
  std::uniform_int_distribution<int32_t> i8dist = std::uniform_int_distribution<int32_t>(
      std::numeric_limits<int8_t>::min(), std::numeric_limits<int8_t>::max());
  std::uniform_int_distribution<int32_t> u8dist = std::uniform_int_distribution<int32_t>(
...
using SumTestF16 = SumTestBase<xnn_float16>;
using SumTestF32 = SumTestBase<float>;
using SumTestQS8 = SumTestBase<int8_t>;
using SumTestQU8 = SumTestBase<uint8_t>;

INSTANTIATE_TEST_SUITE_P(KeepDims, SumTestF16, testing::Bool());
INSTANTIATE_TEST_SUITE_P(KeepDims, SumTestF32, testing::Bool());
INSTANTIATE_TEST_SUITE_P(KeepDims, SumTestQS8, testing::Bool());
INSTANTIATE_TEST_SUITE_P(KeepDims, SumTestQU8, testing::Bool());

TEST_F(SumTestF16, define)
{
...
TEST_P(SumTestF16, matches_operator_api) {
  ...
  // Call operator API.
  const xnn_status status = xnn_create_sum_nd_f16(flags, &op);
  ...
  ASSERT_EQ(xnn_status_success, xnn_setup_sum_nd_f16(op, workspace.data(), input.data(), operator_output.data()));
  ...
  ASSERT_EQ(xnn_status_success,
            xnn_define_static_sum(subgraph, reduction_axes.size(),
...
TEST_P(SumTestF32, matches_operator_api) {
  bool keep_dims = GetParam();
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  uint32_t flags = keep_dims ? XNN_FLAG_KEEP_DIMS : 0;
  // Call operator API.
  const xnn_status status = xnn_create_sum_nd_f32(flags, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ASSERT_EQ(xnn_status_success, status);
  ...
  ASSERT_EQ(xnn_status_success,
    xnn_reshape_sum_nd_f32(op,
      reduction_axes.size(), reduction_axes.data(),
      input_shape.size(), input_shape.data(),
      /*threadpool=*/nullptr));

  ASSERT_EQ(xnn_status_success, xnn_setup_sum_nd_f32(op, input.data(), operator_output.data()));
...
TEST_P(SumTestQS8, matches_operator_api) {
  bool keep_dims = GetParam();
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  uint32_t flags = keep_dims ? XNN_FLAG_KEEP_DIMS : 0;
  // Call operator API.
  const xnn_status status = xnn_create_sum_nd_qs8(
  ...
  ASSERT_LE(workspace_alignment, XNN_ALLOCATION_ALIGNMENT);
  std::vector<char, AlignedAllocator<char, XNN_ALLOCATION_ALIGNMENT>> workspace(workspace_size);
  ASSERT_EQ(xnn_status_success, xnn_setup_sum_nd_qs8(op, workspace.data(), input.data(), operator_output.data()));
...
TEST_P(SumTestQU8, matches_operator_api) {
  bool keep_dims = GetParam();
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
  ...
  uint32_t flags = keep_dims ? XNN_FLAG_KEEP_DIMS : 0;
  // Call operator API.
  const xnn_status status = xnn_create_sum_nd_qu8(
      input_scale / output_scale, input_zero_point, output_zero_point, flags, &op);
  if (status == xnn_status_unsupported_hardware) {
    GTEST_SKIP();
  }
  ...
  ASSERT_LE(workspace_alignment, XNN_ALLOCATION_ALIGNMENT);
  std::vector<char, AlignedAllocator<char, XNN_ALLOCATION_ALIGNMENT>> workspace(workspace_size);
  ASSERT_EQ(xnn_status_success, xnn_setup_sum_nd_qu8(op, workspace.data(), input.data(), operator_output.data()));
...
TEST_F(SumTestF32, reshape_output_keep_dims)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(SumTestF32, reshape_output_no_keep_dims)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(SumTestQS8, reshape_output_keep_dims)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(SumTestQS8, reshape_output_no_keep_dims)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(SumTestQU8, reshape_output_keep_dims)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
...
TEST_F(SumTestQU8, reshape_output_no_keep_dims)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));
