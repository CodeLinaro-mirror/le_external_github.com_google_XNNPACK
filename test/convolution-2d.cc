...
using ConvolutionTestQD8F16QC8W = QuantizedConvolutionTestBase<float, int8_t, float, xnn_float16_t>;
...
using ConvolutionTestF16 = ConvolutionTestBase<xnn_float16_t, float, float>;
...
TEST_F(ConvolutionTestQD8F16QC8W, internally_allocated_dynamic_quantization_parameters)
{
  ...
  std::unique_ptr<xnn_subgraph, decltype(&xnn_delete_subgraph)> auto_subgraph(subgraph, xnn_delete_subgraph);
  uint32_t input_id = XNN_INVALID_NODE_ID;
  std::vector<xnn_float16_t> convert_input(batch_size * input_height * input_width * groups * group_input_channels + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
