...
class DWConv2DMicrokernelTester {
 public:
  ...
  void Test(xnn_f16_dwconv2d_chw_ukernel_fn dwconv, xnn_init_f16_minmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> input(input_height() * input_width() + 2 * XNN_EXTRA_BYTES);
    std::vector<xnn_float16_t> zero(input_width() + 2 * XNN_EXTRA_BYTES);
    std::vector<xnn_float16_t> packed_weights(kernel_size() + 1);
    std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> output(output_height() * output_width());
    std::vector<float> output_ref(output_height() * output_width());
