...
struct UnaryOpInfo {
  ...
  virtual float ReferenceImpl(float x, const xnn_unary_params& params) const = 0;
  virtual int ReferenceImpl(int x, const xnn_unary_params& params) const {
    float f = ReferenceImpl(static_cast<float>(x), params);
    f = std::max<float>(f, std::numeric_limits<int>::min());
    f = std::min<float>(f, std::numeric_limits<int>::max());
