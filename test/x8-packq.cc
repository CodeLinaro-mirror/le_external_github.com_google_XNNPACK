...
TEST(X8_PACKQ_F32QP8__SCALAR_U1, k_div_kr_m_gt_mr) {
  ...
  TEST(X8_PACKQ_F32QP8__NEON_U2, k_div_kr_m_div_mr) {
    ...
    for (size_t kr = 1; kr <= 4; kr++) {
      for (size_t mr = 1; mr <= 4; mr++) {
        PackQMicrokernelTester()
            ...
            .mr(mr)
            .kr(kr)
            .Test(xnn_x8_packq_f32qp8_ukernel__neon_u2);
      }
    }
  }

  TEST(X8_PACKQ_F32QP8__NEON_U2, k_div_kr_m_div_mr_kr_div_sr) {
    TEST_REQUIRES_ARM_NEON;
    for (size_t sr = 1; sr <= 4; sr++) {
      for (size_t kr = sr; kr <= 4 * sr; kr += sr) {
        for (size_t mr = 1; mr <= 4; mr++) {
  ...
  TEST(X8_PACKQ_F32QP8__NEON_U2, k_div_kr_m_lt_mr) {
    TEST_REQUIRES_ARM_NEON;
    for (size_t kr = 1; kr <= 4; kr++) {
      for (size_t mr = 2; mr <= 4; mr++) {
        PackQMicrokernelTester()
  ...
  TEST(X8_PACKQ_F32QP8__NEON_U2, k_div_kr_m_gt_mr) {
    TEST_REQUIRES_ARM_NEON;
    for (size_t kr = 1; kr <= 4; kr++) {
      for (size_t mr = 2; mr <= 4; mr++) {
        PackQMicrokernelTester()
