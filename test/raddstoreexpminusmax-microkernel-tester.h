...
class RAddStoreExpMinusMaxMicrokernelTester {
 public:
  ...
  void Test(xnn_f16_raddstoreexpminusmax_ukernel_fn raddstoreexpminusmax, xnn_init_f16_expminus_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> x(elements() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> y(elements());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const xnn_float16_t x_max_as_half = x_max_as_float;
      for (size_t i = 0; i < elements(); i++) {
      ...
      xnn_float16_t sum = std::nanf("");
      xnn_f16_expminus_params params;
      if (init_params) {
        init_params(&params);
      }
      raddstoreexpminusmax(elements() * sizeof(xnn_float16_t), x.data(), &x_max_as_half, y.data(), &sum, &params);
