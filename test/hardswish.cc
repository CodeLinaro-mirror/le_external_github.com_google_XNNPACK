...
using HardSwishTestF16 = UnaryTest<xnn_float16_t>;
...
{
  std::vector<xnn_float16_t> input(num_output_elements + XNN_EXTRA_BYTES / sizeof(xnn_float16_t), std::nanf(""));
  std::uniform_real_distribution<float> f32dist(-4.0f, 4.0f);
  std::generate(input.begin(), input.end(), [&]() { return f32dist(rng); });
  std::vector<xnn_float16_t> subgraph_output(num_output_elements, std::nanf(""));
  ...
  std::vector<xnn_float16_t> operator_output(num_output_elements, std::nanf(""));
