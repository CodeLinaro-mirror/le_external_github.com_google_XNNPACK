...
class ScaledDotProductAttentionOperatorTester {
 public:
  ...
  void TestF16() const {
    ...
    std::vector<xnn_float16_t> query(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + batch_size() * query_heads() * query_tokens() * query_key_channels());
    std::vector<xnn_float16_t> key(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + batch_size() * key_value_heads() * key_value_tokens() * query_key_channels());
    std::vector<xnn_float16_t> value(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + batch_size() * key_value_heads() * key_value_tokens() * value_channels());
    std::vector<xnn_float16_t> scale(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + query_key_channels());
    std::vector<xnn_float16_t> mask(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + query_tokens() * key_value_tokens());
    std::vector<xnn_float16_t> output(batch_size() * query_heads() * query_tokens() * value_channels());
