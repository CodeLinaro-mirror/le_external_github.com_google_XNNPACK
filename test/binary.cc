...
float compute(xnn_binary_operator op, float a, float b) {
  switch (op) {
    case xnn_binary_add:
      return a + b;
...
int32_t compute(xnn_binary_operator op, int32_t a, int32_t b) {
  switch (op) {
    case xnn_binary_add:
        ...
        for (const auto& i : EnumerateIndices(output.extents())) {
          if (std::is_integral<T>::value) {
            const int32_t expected = std::min<int32_t>(
                std::max<int32_t>(compute(p.op, a(i), b(i)), params.output_min),
                params.output_max);
            ...
            float expected = compute(p.op, a_i, b_i);
            ...
            float expected = compute(p.op, a(i), b(i));
            expected = std::max<float>(expected, params.output_min);
            expected = std::min<float>(expected, params.output_max);
            if (std::isnan(expected)) {
              if (p.op == xnn_binary_pow &&
                  xnn_datatype_of<T>() == xnn_datatype_fp16) {
                // TODO: Figure out why we can't match NaN in this case.
              } else {
