...
class ConvolutionOperatorTester {
 public:
  ...
  void TestNHWCxQD8F16QC8W() const {
    ...
    std::vector<xnn_float16_t> output(
        batch_size() *
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t i = 0; i < output_ref.size(); ++i) {
        ...
        output_ref[i] = xnn_float16_t(output_ref[i]);
      }
      ...
      if (use_weights_cache()) {
        ...
        std::vector<xnn_float16_t> output2(output.size(), UINT16_C(0xDEAD));
        size_t workspace_size = SIZE_MAX;
        size_t workspace_alignment = SIZE_MAX;
        ASSERT_EQ(xnn_status_success,
                  xnn_reshape_convolution2d_nhwc_qd8_f16_qc8w(
  ...
  void TestNHWCxF16() const {
    ...
    std::vector<xnn_float16_t> input(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
      batch_size() * ((input_height() * input_width() - 1) * input_channel_stride() + groups() * group_input_channels()));
    std::vector<xnn_float16_t> kernel(groups() * group_output_channels() * kernel_height() * kernel_width() * group_input_channels());
    std::vector<float> kernel_as_float(kernel.size());
    std::vector<xnn_float16_t> bias(groups() * group_output_channels());
    std::vector<float> bias_as_float(bias.size());
    std::vector<xnn_float16_t> output(batch_size() * ((output_height() * output_width() - 1) * output_channel_stride() + groups() * group_output_channels()));
    std::vector<float> output_ref(batch_size() * output_height() * output_width() * groups() * group_output_channels());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const float scaled_min = xnn_float16_t(accumulated_min + accumulated_range / 255.0f * float(qmin()));
      const float scaled_max = xnn_float16_t(accumulated_max - accumulated_range / 255.0f * float(255 - qmax()));
      ...
      if (use_weights_cache()) {
        ...
        std::vector<xnn_float16_t> output2(output.size(), std::nanf(""));
        size_t workspace_size = SIZE_MAX;
        size_t workspace_alignment = SIZE_MAX;
        ASSERT_EQ(
          xnn_status_success,
  ...
  void VerifyNHWCxF16(const std::vector<xnn_float16_t> &output,
  ...
  void TestNCHWxF16() {
    ...
    std::vector<xnn_float16_t> input(2 * XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
      ((batch_size() - 1) * input_channel_stride() + groups() * group_input_channels()) * input_height() * input_width());
    std::vector<xnn_float16_t> kernel(
      groups() * group_output_channels() * kernel_height() * kernel_width() * group_input_channels());
    std::vector<float> kernel_as_float(kernel.size());
    std::vector<xnn_float16_t> bias(groups() * group_output_channels());
    std::vector<float> bias_as_float(bias.size());
  ...
  void VerifyNCHWxF16(const std::vector<xnn_float16_t> &output,
                      const std::vector<float> &output_ref,
                      const float output_min, const float output_max) const {
    for (size_t i = 0; i < batch_size(); i++) {
      for (size_t y = 0; y < output_height(); y++) {
  ...
  void TestSetupNHWCxF16() const {
    ...
    std::vector<xnn_float16_t> input(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) + std::max(
      batch_size() * ((input_height() * input_width() - 1) * input_channel_stride() + groups() * group_input_channels()),
      next_batch_size() * ((next_input_height() * next_input_width() - 1) * input_channel_stride() + groups() * group_input_channels())));
    std::vector<xnn_float16_t> kernel(groups() * group_output_channels() * kernel_height() * kernel_width() * group_input_channels());
    std::vector<xnn_float16_t> bias(groups() * group_output_channels());
    std::vector<xnn_float16_t> output(std::max(
      batch_size() * ((output_height() * output_width() - 1) * output_channel_stride() + groups() * group_output_channels()),
      next_batch_size() * ((next_output_height() * next_output_width() - 1) * output_channel_stride() + groups() * group_output_channels())));
    std::vector<float> output_ref(batch_size() * output_height() * output_width() * groups() * group_output_channels());
