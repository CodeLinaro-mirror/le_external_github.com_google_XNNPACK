...
class PReLUOperatorTester {
 public:
  ...
  void TestF16() const {
    ...
    std::vector<xnn_float16_t> x((batch_size() - 1) * x_stride() + input_channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> w(input_channels());
    std::vector<float> w_as_float(input_channels());
    std::vector<xnn_float16_t> y((batch_size() - 1) * y_stride() + input_channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      if (use_weights_cache()) {
        ...
        std::vector<xnn_float16_t> y2(y.size(), std::nanf(""));
        ASSERT_EQ(xnn_status_success,
                  xnn_reshape_prelu_nc_f16(
                      prelu_op2,
                      batch_size(),
  ...
  void VerifyF16(const std::vector<xnn_float16_t>& y, const std::vector<float>& y_ref) const {
    for (size_t i = 0; i < batch_size(); i++) {
      for (size_t c = 0; c < input_channels(); c++) {
        ASSERT_NEAR(
            y[i * y_stride() + c],
