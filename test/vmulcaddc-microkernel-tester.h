...
class VMulCAddCMicrokernelTester {
 public:
  ...
  void Test(xnn_f16_vmulcaddc_ukernel_fn vmulcaddc, xnn_init_f16_minmax_params_fn init_params) const {
    ...
    std::vector<xnn_float16_t> x((rows() - 1) * input_stride() + channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> scale(channels());
    std::vector<xnn_float16_t> bias(channels());
    std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> packed_w(packed_channels() * 2);
    std::vector<xnn_float16_t> y((rows() - 1) * output_stride() + channels() + (inplace() ? XNN_EXTRA_BYTES / sizeof(xnn_float16_t) : 0));
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      const xnn_float16_t* x_data = inplace() ? y.data() : x.data();
      ...
      const float y_max = xnn_float16_t(accumulated_max - accumulated_range / 255.0f * float(255 - qmax()));
      const float y_min = xnn_float16_t(accumulated_min + accumulated_range / 255.0f * float(qmin()));
      ...
      vmulcaddc(rows(), channels() * sizeof(xnn_float16_t),
        x_data, input_stride() * sizeof(xnn_float16_t),
        packed_w.data(),
        y.data(), output_stride() * sizeof(xnn_float16_t),
        &params);
