...
class SoftMaxOperatorTester {
 public:
  ...
  void TestF16() const {
    ...
    std::vector<xnn_float16_t> input((batch_size() - 1) * input_stride() + channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
    std::vector<xnn_float16_t> output((batch_size() - 1) * output_stride() + channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
