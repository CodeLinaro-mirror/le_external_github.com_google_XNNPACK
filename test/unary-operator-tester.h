...
struct UnaryOpTestParams {
  ...
  std::string ToString() const {
    ...
    } else if (!input_stride.has_value() && output_stride.has_value()) {
  ...
  std::optional<size_t> input_stride;
  std::optional<size_t> output_stride;
  std::optional<uint8_t> qmin;
  std::optional<uint8_t> qmax;
  std::optional<LoopLimits<float>> input_scale;
  std::optional<LoopLimits<int32_t>> input_zero_point;
};

inline std::ostream& operator<<(std::ostream& os, UnaryOpTestParams params) {
  os << "{test_name: '" << params.test_name
     << "', batch_size: " << params.batch_size;
  if (params.input_stride.has_value()) {
    os << ", input_stride: " << *params.input_stride;
  }
  if (params.output_stride.has_value()) {
    os << ", output_stride: " << *params.output_stride;
  }
  if (params.qmin.has_value()) {
    os << ", qmin: " << *params.qmin;
  }
  if (params.qmax.has_value()) {
    os << ", qmax: " << *params.qmax;
  }
  if (params.input_scale.has_value()) {
    os << ", input_scale: " << params.input_scale->ToString();
  }
  if (params.input_zero_point.has_value()) {
...
#define CREATE_UNARY_TEST(datatype, Tester)                           \
  ...
  TEST_P(Tester##datatype, Test##datatype) {                          \
    ...
    for (size_t channels = test_case.channels.min;                    \
      ...
      if (test_case.input_scale.has_value()) {                        \
      ...
      for (float input_scale = input_scale_limits.min;                \
        ...
        if (test_case.input_zero_point.has_value()) {                 \
        ...
        for (int32_t input_zero_point = input_zero_point_limits.min;  \
          ...
          if (test_case.input_stride.has_value()) {                   \
            tester.input_stride(*test_case.input_stride);             \
          }                                                           \
          if (test_case.output_stride.has_value()) {                  \
            tester.output_stride(*test_case.output_stride);           \
          }                                                           \
          if (test_case.qmin.has_value()) {                           \
            tester.qmin(*test_case.qmin);                             \
          }                                                           \
          if (test_case.qmax.has_value()) {                           \
