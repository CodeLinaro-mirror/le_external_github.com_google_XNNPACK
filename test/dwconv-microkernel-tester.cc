...
void DWConvMicrokernelTester::Test(
  ...
  std::vector<const xnn_float16_t*> indirection((width() - 1) * step() +
                                           kernel_tile());
  std::vector<xnn_float16_t> input(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
                              indirection.size() * channels());
  std::vector<xnn_float16_t> kernel(channels() * kernel_tile());
  std::vector<xnn_float16_t> bias(channels());
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> packed_weights(
      (kernel_tile() + 1) * packed_channels());
  std::vector<xnn_float16_t> zero(channels() + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<xnn_float16_t> output((width() - 1) * output_stride() + channels());
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    const float output_min = xnn_float16_t(
        accumulated_min +
        accumulated_range / 255.0f * static_cast<float>(qmin()));
    const float output_max = xnn_float16_t(
        accumulated_max -
    ...
    dwconv_minmax(channels(), width(),
                  reinterpret_cast<const xnn_float16_t**>(indirection.data()),
                  packed_weights.data(), output.data(), step() * sizeof(void*),
                  (output_stride() - channels()) * sizeof(xnn_float16_t),
                  input_offset() * sizeof(xnn_float16_t), zero.data(), &params);
...
void DWConvMicrokernelTester::Test(
  ...
  const size_t tile_size = xnn_dwconv_multipass_tile_size(
      kernel_size(), first_pass_tile(), middle_pass_tile(), last_pass_tile());
  std::vector<const xnn_float16_t*> indirection((width() - 1) * step() + tile_size);
  std::vector<xnn_float16_t> input(XNN_EXTRA_BYTES / sizeof(xnn_float16_t) +
                              indirection.size() * channels());
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> buffer(
      XNN_MULTIPASS_EXTRA_BYTES / sizeof(xnn_float16_t) + channels());
  std::vector<xnn_float16_t> kernel(channels() * kernel_size());
  std::vector<xnn_float16_t> bias(channels());
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> packed_weights(
      xnn_dwconv_multipass_weights_size(tile_size, channels(), channel_tile(),
                                        channel_subtile(), channel_round(),
                                        /*bias_element_size=*/sizeof(xnn_float16_t),
  ...
  for (size_t iteration = 0; iteration < iterations(); iteration++) {
    ...
    dwconv_minmax(channels(), width(),
                  ...
                  input_stride_elements * sizeof(void*),
                  (output_stride() - channels()) * sizeof(xnn_float16_t),
                  input_offset() * sizeof(xnn_float16_t), zero.data(), kernel_size(),
                  buffer.data(), &params);
