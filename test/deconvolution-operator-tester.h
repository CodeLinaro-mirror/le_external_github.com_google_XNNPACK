...
class DeconvolutionOperatorTester {
 public:
  ...
  void TestF16() const {
    ...
    std::vector<xnn_float16_t> output((batch_size() * output_height() * output_width() - 1) * output_pixel_stride() + groups() * group_output_channels());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      output_min = xnn_float16_t(output_min);
      output_max = xnn_float16_t(output_max);
