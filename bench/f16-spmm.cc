...
static inline bool is_fp16_zero(xnn_float16_t x) {
  const xnn_float16_t two_x = x + x;
  return two_x == 0;
}
  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> input(kc * mc);
  // Think of b as (n/nr + n % nr) x k, expansion happens later.
  const size_t ncols = nc / nr + nc % nr;
  std::vector<xnn_float16_t> b(ncols * kc);
  std::vector<xnn_float16_t> bias(nc);
  ...
  std::vector<xnn_float16_t> w(nc * kc + nc);
  std::vector<xnn_float16_t> output(nc * mc);
  ...
  for (size_t nn = 0; nn < nc / nr; nn++) {
    ...
    for (size_t kk = 0; kk < kc; kk++) {
      if (!is_fp16_zero(b[nn * kc + kk])) {
        ...
        for (size_t i = 0; i < nr; ++i)
          w[wcnt++] = xnn_float16_t(b[nn * kc + kk]) + static_cast<float>(i);
        // Skip the very first non-zero weight as we record only the difference.
        if (first_nzz) {
          first_kk = kk;
        } else {
          const int32_t increment = int32_t(kk - last_kk) * int32_t(mc * sizeof(xnn_float16_t));
          dmap[nnz++] = increment;
        }
        last_kk = kk;
        first_nzz = false;
  ...
  const int64_t increment = int32_t(first_kk - last_kk) * int32_t(mc * sizeof(xnn_float16_t));
  dmap[nnz++] = increment;
  ...
  std::vector<xnn_float16_t> b_full(nc * kc);
  if (nr == 1) {
     b_full = b;
  }
  else {
    for (size_t nn = 0; nn < nc / nr; nn++) {
      for (size_t kk = 0; kk < kc; kk++) {
        if (b[nn * kc + kk] != 0.0f) {
          for (size_t i = 0; i < nr; ++i)
            b_full[nr * nn * kc + i * kc + kk] = xnn_float16_t(
  ...
  for (auto _ : state) {
    ...
    spmm(mc * sizeof(xnn_float16_t), nc,
      input.data() + first_kk * mc,
      w.data(), dmap.data(), nmap.data(),
      output.data(), mc * sizeof(xnn_float16_t),
      &params);
  }
