  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  const size_t packed_elements = benchmark::utils::RoundUp(elements, cache_line_size_max / sizeof(xnn_float16_t));
  ...
  const size_t num_buffers = 1 +
    benchmark::utils::DivideRoundUp<size_t>(benchmark::utils::GetMaxCacheSize(), packed_elements * sizeof(xnn_float16_t));
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> x(elements);
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> y(packed_elements * num_buffers);
  ...
  for (auto _ : state) {
    state.PauseTiming();
    xnn_float16_t x_max = std::nanf("");
    rmax(elements * sizeof(xnn_float16_t), x.data(), &x_max, /*params=*/nullptr);
    if (++buffer_index == num_buffers) {
      buffer_index = 0;
    }
    state.ResumeTiming();

    xnn_float16_t y_sum = std::nanf("");
    raddstoreexpminusmax(elements * sizeof(xnn_float16_t), x.data(), &x_max, y.data() + buffer_index * packed_elements, &y_sum, &params);
  }
  ...
  const size_t bytes_per_iteration = 2 * elements * sizeof(xnn_float16_t);
  ...
  BENCHMARK_CAPTURE(f16_raddstoreexpminusmax, neonfp16arith_rr2_p2_u32,
    ...
    ->Apply(benchmark::utils::UnaryElementwiseParameters<xnn_float16_t, xnn_float16_t>)
    ->UseRealTime();
