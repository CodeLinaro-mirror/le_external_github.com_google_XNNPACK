    ...
    benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> input(rows * batch + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<xnn_float16_t> output(rows);
  ...
  for (auto _ : state) {
    for (int i = 0; i < rows; ++i) {
      rsum(batch * sizeof(xnn_float16_t), &input[i * batch], &output[i], &params);
    }
  }
    ...
    benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> input(rows * channels + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<float> output(channels);
  std::vector<xnn_float16_t> zero(channels + XNN_EXTRA_BYTES / sizeof(xnn_float16_t), 0);
  std::iota(input.begin(), input.end(), 0.0f);
  ...
  for (auto _ : state) {
    rdsum(rows, channels, input.data(), channels * sizeof(xnn_float16_t), zero.data(), output.data(), &params);
  }
