...
bazel run -c opt //tbench:f32_gemm_bench -- $(cat tbench/models/mobilenet_v2.txt)

# Run a custom Convolution shape (H W KH KW PH PW S D GCin GCout)
bazel run -c opt //tbench:f32_conv_hwc_bench -- 224 224 3 3 1 1 2 1 3 32
```
...
```bash
bazel run -c opt ///ench:qd8_f32_qc8w_gemm_bench -- --block_size=128 $(cat bench/models/llm_gemm.txt)
```
*Default value is 32.*
