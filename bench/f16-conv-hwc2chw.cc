  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t> input(input_height * input_width * input_channels + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::generate(input.begin(), input.end(), f32rng);
  std::vector<xnn_float16_t> kernel(output_channels * kernel_size * kernel_size * input_channels);
  ...
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> zero(input_channels * input_width + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  ...
  const size_t num_buffers = 1 +
    benchmark::utils::DivideRoundUp<size_t>(benchmark::utils::GetMaxCacheSize(),
      sizeof(xnn_float16_t) * (weights_elements + output_elements));

  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> packed_weights(weights_elements * num_buffers);
  ...
  std::vector<xnn_float16_t> output(output_elements * num_buffers);
  ...
  for (auto _ : state) {
    state.PauseTiming();
    benchmark::utils::PrefetchToL1(input.data(), input.size() * sizeof(xnn_float16_t));
    ...
    conv(
      ...
      output_channels * output_width * sizeof(xnn_float16_t),
      output_channels * sizeof(xnn_float16_t),
      &params);
  }
