  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> input(elements);
  ...
  xnn_float16_t output[2] = {std::nanf(""), std::nanf("")};
  for (auto _ : state) {
    rminmax(elements * sizeof(xnn_float16_t), input.data(), output, &params);
  }
  ...
  const size_t bytes_per_iteration = elements * sizeof(xnn_float16_t);
  ...
  BENCHMARK_CAPTURE(f16_rminmax, neonfp16arith_u8,
    ...
    ->Apply(benchmark::utils::ReductionParameters<xnn_float16_t>)
    ->UseRealTime();
