  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t> a(channels * input_height * input_width + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::generate(a.begin(), a.end(), f32rng);
  std::vector<xnn_float16_t> k(channels * kernel_height * kernel_width);
  std::generate(k.begin(), k.end(), f32rng);
  std::vector<xnn_float16_t> b(channels);
  std::generate(b.begin(), b.end(), f32rng);

  std::vector<xnn_float16_t> z(channels + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  ...
  const size_t num_buffers = 1 +
    benchmark::utils::DivideRoundUp<size_t>(benchmark::utils::GetMaxCacheSize(),
      sizeof(xnn_float16_t) * (w_elements + c_elements) + sizeof(void*) * i_elements);

  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> w(w_elements * num_buffers);
  ...
  std::vector<const xnn_float16_t*> i(i_elements * num_buffers);
  ...
  std::vector<xnn_float16_t> c(c_elements * num_buffers);
  ...
  for (auto _ : state) {
    state.PauseTiming();
    benchmark::utils::PrefetchToL1(a.data(), a.size() * sizeof(xnn_float16_t));
    buffer_index = (buffer_index + 1) % num_buffers;
    state.ResumeTiming();

    for (size_t y = 0; y < output_height; y++) {
      dwconv(channels, output_width,
        reinterpret_cast<const xnn_float16_t**>(i.data() + buffer_index * i_elements + step_height * y),
  ...
  state.counters["bytes"] = benchmark::Counter(
    uint64_t(state.iterations()) * (output_size + input_height * input_width + kernel_size + 1 /* bias */) * channels * sizeof(xnn_float16_t),
  ...
  benchmark::utils::IsaCheckFunction isa_check = nullptr)
{
  ...
  std::vector<xnn_float16_t> a(channels * input_height * input_width + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::generate(a.begin(), a.end(), f32rng);
  std::vector<xnn_float16_t> k(channels * kernel_size);
  std::generate(k.begin(), k.end(), f32rng);
  std::vector<xnn_float16_t> b(channels);
  std::generate(b.begin(), b.end(), f32rng);

  std::vector<xnn_float16_t> z(channels + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  std::vector<xnn_float16_t, AlignedAllocator<xnn_float16_t, 64>> buffer(channels + XNN_MULTIPASS_EXTRA_BYTES / sizeof(xnn_float16_t));
  ...
  const size_t w_elements =
    xnn_dwconv_multipass_weights_size(
      tile_size, channels, channel_tile, channel_subtile, channel_round, /*bias_element_size=*/sizeof(xnn_float16_t),
      /*log2_filter_element_size=*/1, /*extra_weights_byte=*/0) /
    sizeof(xnn_float16_t);
  // Can read (primary_tile - kernel_size) elements after end of indirection buffer.
