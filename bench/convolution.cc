...
void xnnpack_convolution_f16(benchmark::State& state, const char* net) {
  ...
  std::vector<xnn_float16_t> input(batch_size * input_height * input_width * input_pixel_stride + XNN_EXTRA_BYTES / sizeof(xnn_float16_t));
  ...
  const size_t num_buffers = 1 +
    benchmark::utils::DivideRoundUp<size_t>(benchmark::utils::GetMaxCacheSize(),
      sizeof(xnn_float16_t) * (kernel.size() + bias.size() + output_elements));
